...
function WellnessTrips() {
  ...
  const fetchTrips = async () => {
    try {
      ...
      // Filter active trips with Wellness category
      const wellnessTrips = (response.trips || []).filter(trip => {
        const isActive = trip.status === 'active'
        const categories = Array.isArray(trip.category) ? trip.category : []
        return isActive && categories.includes('Wellness')
      })
      setTrips(wellnessTrips)
    } catch (error) {
      ...
      <div className="mx-auto w-full max-w-7xl px-4 md:px-6 lg:px-8">
        <div className="mb-8 md:mb-12">
          ...
          <p className="text-gray-600 text-lg max-w-3xl">
            Discover rejuvenating wellness experiences, yoga retreats, spa therapies, and holistic healing destinations across India.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No wellness trips available at the moment.</p>
          </div>
        )}
      </div>
    </div>
